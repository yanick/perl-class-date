...
function, or by calling the C<Class::Date-E<gt>new()> constructor.
...
to add 1900 (or possibly 1904) to the value first.  Use the C<now()>
function, or see below for a more reliable method.
...
$Class::Date::LOCAL_TIMEZONE>, and C<gmdate $x> is equivalent to C<date
$x, 'UTC'>
...
not to any country's time zone:

  $date = date("2001-01-17 10:10:00 +12:00");
  print $date->tz;   # prints "+12:00";

However, such dates may freely be converted to a local time zone using
the C<to_tz($timezone)> copy constructor, or the C<set(tz =E<gt>
$timezone)> method:
...
resolve this ambiguity, B<this method of construction is only valid
with the XS version of this module>.  You must only use "clean"
numbers that have not been converted to a string.  To force this, it
is recommended that you explicitly force integer interpretation by
prepending C<0+> to the time (or using C<int()>).
...
#=====================================================================
#   API REVIEWED UP TO HERE
#=====================================================================

# this epoch vs ISO-8601 thing is a bit concerning.
