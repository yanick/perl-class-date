...
Since 1.0.11, Class::Date handle timezones natively.
...
Class::Date 1.1.0 unfortunately does not work on win32 platform. There are
some issues with it and I currently don't know how to solve it.

Class::Date 1.0.10 or earlier works in win32 with no known problems. You
can acquire and compile yourself from source or you can get the binary from
the Chris Winters' .ppd repository with the following commands:
...
The first two steps in PPM only need to be done at the first time. Next
